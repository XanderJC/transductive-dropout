...
class mc_dropout:
    ...
    def objective_train(self, params, t):
        ...
        return np.sum(
            (self.forward_pass(params, self.train_x) - self.train_y) ** 2
    ...
    def train(self, train_x, train_y, iters):
        ...
        optimised_params = adam(
            grad(self.objective_train),
            init_params,
            step_size=0.01,
            num_iters=iters,
...
class dropout_regularised:
    def __init__(self, layers):
        ...
        self.params = (self.init_random_params(layer_sizes=layers), self.d_rate)
        self.rate_change = []
        self.params_history = []
...
class transductive(dropout_regularised):
    def __init__(self, layers, d_units=16):
        ...
        self.params = self.init_random_params(layer_sizes=layers, d_size=d_units)
        self.rate_change = []
        self.params_history = []

    @staticmethod
    def init_random_params(layer_sizes, d_size, scale=0.1, rs=npr.RandomState(0)):
        ...
        d_pars = [
            [rs.randn(1, d_size) * scale, rs.randn(d_size) * scale],
            [
